...
turns these sections into Telegram buttons.
"""
...
# Ordered by the product priority (public first, then treasurer, then admin).
...
# Permission gating each section.
_SECTION_PERMISSION: dict[Section, Permission] = {
...
# i18n key and callback token for each section (used by the keyboard builder).
...
def main_sections(actor: StaffUser | None) -> list[Section]:
    ...
    return [s for s in _SECTION_ORDER if _SECTION_PERMISSION[s] in granted]
